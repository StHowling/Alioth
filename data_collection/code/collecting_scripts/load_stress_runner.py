...
workload_host_addr = '71.12.106.126'

stress_pattern = re.compile(r'([a-zA-Z\-]+)')

def _wait_and_record_stress(wf):
    wf.write('start at %d\n' % time.time())
...
def random_double_stress(base_url1, base_url2, cmd_name, retry_stop, wf, stress_code):
    duration = -1
    if not retry_stop:
        payload = [cmd_name, '-%s' % stress_code, '0']
        resp = requests.post(base_url1, data=json.dumps(payload))
        if resp.status_code != 200:
            print('failed to start stress')
            return duration, START_FAILED
        resp = requests.post(base_url2, data=json.dumps(payload))
        if resp.status_code != 200:
            # best effort to stop stress in the first stress server
            requests.delete('%s?command=%s&force=true' % (base_url1, cmd_name))
            print('failed to start stress')
            return duration, START_FAILED
        duration = _wait_and_record_stress(wf)
    return _stop_stress([base_url1, base_url2], cmd_name, duration)
...
def stress_constant_start(stress_code, stress_vm, duration=None):
    if stress_code == '0':
        return
    ...
    for i in range(len(stress_vm_list)):
        ...
        if duration:
            log_url = 'http://%s:8081/v1/cmd' % stress_vm_list[i]
            payload = ['sysstat_log', '/root/log-stressor-stats.py', stress_code, duration]
            resp = requests.post(log_url, data=json.dumps(payload))
            if resp.status_code != 200:
                print('failed to log system stats at %s' % stress_vm_list[i])


def stress_constant_stop(stress_code, stress_vm):
    cmd_name = 'stress'
    stress_vm_list = stress_cpunum_map[stress_vm]
    for i in range(len(stress_vm_list)):
...
def redis_run(loadlevel='2'):
    ...
    num = 12000000+min(int(loadlevel),12)*5500000
...
#    p.wait()
    ...
    with open('%s-sysbench-result.log' % current, 'w') as f:
        p = subprocess.Popen([
            '/root/sysbench', '--num-threads=1', '--max-requests=45000',
            '--test=oltp', '--db-driver=mysql', '--mysql-host=192.168.0.233',
            '--mysql-port=3306', '--mysql-user=sysbench',
...
#    kafka_cold_run() 
    ...
    with open('%s-kafka-result.log' % current, 'w') as f:
        p = subprocess.Popen([cmd_path,
                              '--throughput', '-1', # -1 mean no control client maximum throughput
                              '--topic', topic,
...
def mongodb_run(loadlevel='5'):
    ...
    workload_path = '/root/mongodb-workload'
    op_count = 13700000
