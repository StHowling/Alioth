...
class MonitoringCore(Monitoring):
    ...
    def print_data(self):
        ...
        print("    CORE    LLC[KB]    MBL[MB]    MBR[MB]    IPC")#              IPC_RETIRED               IPC_UNHALTED") #Note: these data corresponds to un-nop/stall cycles and retired instructions per sec, which are extremely large
        ...
        #    ipcr = group.values.ipc_retired
         #   ipcu = group.values.ipc_unhalted
            print("%8u %10.1f %10.1f %10.1f %6.2f" % (core, llc, mbl, mbr, ipc))
...
if __name__ == "__main__":
    core_list = sys.argv[1].split(',')
    for i in range(len(core_list)):
        core_list[i] = int(core_list[i])
    main(core_list, int(sys.argv[2]))
